...
def main():
    ...
    parser.add_argument('-d', '--directory', help='Directory containing the outputs of a FreeSurfer reconall pipeline', required=True)
    ...
    print(path)
    der_path = path+"/**/"+baseFn
    print(der_path)
    fns = sorted(glob.glob(der_path, recursive=True))
    print(fns)
    newRows = []
    ...
    for fn in fns:
        ...
        scanId = os.path.dirname(fn.replace('/stats/','/' )).split("/")[-2]
        if fn.endswith("/"):
